...
function generateComparison(word: string, zh: string): string {
  ...
  // 最终兜底：给出具体操作建议
  const z = primaryZh(zh);
  const syllables = w.match(/[aeiouy]+/g) || [w];
  if (syllables.length >= 2) {
    return `🔊 拆音节读：${syllables.join('-')}，每个音节对应"${z}"的一部分，边读边想意思`;
  }
  return `🔊 拆字母读：${w.split('').join('-')}，逐字母拼读联想"${z}"`;
}
...
/** 为单词生成有具体语境的例句场景，而非空洞的「想象你在XX」 */
function generateStory(word: string, zh: string): string {
  const w = word.toLowerCase();
  const h = hashStr(w);

  // 按词性给出不同场景
  const posHints: Array<(w: string, z: string) => string> = [
    // 1. 对话型场景（适合口语高频词）
    (w, z) => `🗣 对话中用："Could you help me with ${w}?"（能帮我${z}吗？）—— 在英语国家求助时说出口，对方秒懂`,
    (w, z) => `🗣 表达观点："I think ${w} is important."（我觉得${z}很重要）—— 在讨论中自然表达`,
    (w, z) => `🗣 描述日常："I need ${w} every day."（我每天都需要${z}）—— 把单词绑定到日常生活`,
    // 2. 阅读型场景
    (w, z) => `📖 阅读中遇到：看到 "${w}" 时先猜意思为"${z}"，再查证——猜对的成就感会加深记忆`,
    (w, z) => `📖 写作中用：下次写英文邮件或日记时，刻意用一次 "${w}"（${z}）—— 用过的词不会忘`,
    // 3. 发音绑定场景
    (w, z) => `🔊 发音绑定：${w} 的发音中藏着 "${z}" 的线索——每次读准这个音，就想起意思`,
    (w, z) => `🔊 韵律记忆：${w} 和 "${z}" 押韵/谐音——用声音的相似性把中英文绑在一起`,
    // 4. 对比场景
    (w, z) => `⚖️ 对比记忆：${w}（${z}）和它的反义/近义词一起记——成对记忆效率翻倍`,
    (w, _z) => `⚖️ 词族扩展：${w} 的词根/词缀关系——掌握一个词根，解锁一串单词`,
  ];

  return posHints[h % posHints.length](w, zh);
}
...
function generateAssociation(word: string, zh: string, morph: Morphology): string {
  ...
  if (w.length <= 4) {
    const h = hashStr(w);
    const strategies = [
      `🔊 拼读记忆：${w.split('').join('-')}，逐字母拼读"${z}"——短词拼写即发音，多拼几遍就记住`,
      `🔊 谐音锁定："${w}" 读起来像 "${z}" 的中文近似音——声音绑定，听到这个音就想起意思`,
      `🔊 首字母联想：${w[0].toUpperCase()} 代表 "${z}" 的关键特征——用首字母做锚点记全词`,
      `🔊 押韵归类：${w} 和你已知的押韵词放一起记——声音相似的词容易形成记忆链`,
    ];
  ...
  if (syls.length >= 2) {
    return `🔊 拆音节读：${syls.join('-')}，每个音节的发音都藏着"${z}"的线索——边读边想意思`;
  }
  return `🔊 拼读联想：${w.split('').join('-')}，逐字母拼读"${z}"——发音和拼写互相印证`;
}
